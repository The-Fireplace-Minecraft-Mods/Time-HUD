...
public class ClockEvents {
	...
	public static void guiRender(TickEvent.RenderTickEvent t){
		...
		if(ConfigValues.IGHUD && ((mc.inGameHasFocus && !mc.gameSettings.showDebugInfo) || mc.currentScreen instanceof GuiClockMoving) && (!ConfigValues.NEEDCLOCK || hasClock())) {
		...
		if(ConfigValues.REAL && ((mc.inGameHasFocus && !mc.gameSettings.showDebugInfo) || mc.currentScreen instanceof GuiClockMoving)) {
			GlStateManager.pushMatrix();
			ScaledResolution res = new ScaledResolution(mc);
			int width = res.getScaledWidth();
			int height = res.getScaledHeight();
			...
			switch(ConfigValues.RXALIGNMENT){
				case LEFT:
					xPos = ConfigValues.RCLOCKX;
					if(twoline)
						xPos2=xPos;
					break;
				case RIGHT:
					xPos = width-ConfigValues.RCLOCKX;
					if(twoline)
					...
					break;
				case CENTER:
					xPos = width/2+ ConfigValues.RCLOCKX;
					if(twoline)
			...
			switch(ConfigValues.RYALIGNMENT){
				case TOP:
					yPos = ConfigValues.RCLOCKY;
					break;
				case BOTTOM:
					yPos = height-ConfigValues.RCLOCKY;
					break;
				case CENTER:
					yPos = height/2+ConfigValues.RCLOCKY;
			}

			GlStateManager.scale(ConfigValues.RFONTSCALE, ConfigValues.RFONTSCALE, 0);

			mc.ingameGUI.drawString(mc.fontRenderer, d3[0], (int)(xPos/ConfigValues.RFONTSCALE), (int)(yPos/ConfigValues.RFONTSCALE), ConfigValues.RFONTCOLOR);
			if(twoline)
				mc.ingameGUI.drawString(mc.fontRenderer, d3[1], (int)(xPos2/ConfigValues.RFONTSCALE), (int)((yPos/ConfigValues.RFONTSCALE) + mc.fontRenderer.FONT_HEIGHT + mc.fontRenderer.FONT_HEIGHT/3), ConfigValues.RFONTCOLOR);
